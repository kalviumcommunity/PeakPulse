...
export async function seedDemoDataset(scenario: string = 'dinner_crisis'): Promise<DemoDatasetSummary> {
  ...
  try {
    ...
    // 2. Truncate previous demonstration records cleanly
    await client.query(`
      TRUNCATE TABLE refunds, complaints, rider_assignments, deliveries, riders, restaurants, customers RESTART IDENTITY CASCADE;
    `);

    // 3. Seed 8 Diverse Restaurants in Batch
    const resRes = await client.query(`
      ...
      RETURNING id;
    `);
    const restaurantIds = resRes.rows.map(r => r.id);

    // 4. Seed 12 Couriers in Batch
    const rdRes = await client.query(`
      ...
      RETURNING id;
    `);
    const riderIds = rdRes.rows.map(r => r.id);

    // 5. Seed 20 Customers in Batch
    const custValues: string[] = [];
    for (let c = 1; c <= 20; c++) {
      custValues.push(`('Customer ${c}', '+1-555-02${String(c).padStart(2, '0')}', 'customer${c}@example.com', '${100 + c * 10} Main St', ${40.7128 + c * 0.005}, ${-74.0060 + c * 0.005})`);
    }
    ...
    // 6. Generate 520 Deliveries in High-Speed Batches
    const baseDate = new Date();
    baseDate.setHours(0, 0, 0, 0);

    const zones = [
      { name: 'Downtown - Zone A', rests: [2, 3] },
      { name: 'Midtown - Zone B', rests: [4, 5] },
      { name: 'Uptown - Zone C', rests: [0, 1] },
      { name: 'Suburb - Zone D', rests: [7] },
      { name: 'East - Zone E', rests: [6] },
      { name: 'West - Zone F', rests: [2, 4] }
    ];

    const deliveryRows: string[] = [];
    const assignmentRows: string[] = [];
    const complaintRows: string[] = [];
    ...
    for (let i = 1; i <= 520; i++) {
      ...
      const promisedTime = new Date(orderTime.getTime() + 30 * 60000);
      ...
      deliveryRows.push(
        `('ORD-${10000 + i}', ${custId}, ${restId}, ${riderId}, '${orderTime.toISOString()}', '${promisedTime.toISOString()}', '${actualDeliveryTime.toISOString()}', ${orderValue}, ${distanceKm}, 'delivered', ${isBreached}, ${delayMinutes}, '${zoneObj.name}')`
      );

      const assignedAt = new Date(orderTime.getTime() + 3 * 60000);
      const pickedAt = new Date(assignedAt.getTime() + (isTaco ? 22 : 14) * 60000);
      assignmentRows.push(`(${i}, ${riderId}, '${assignedAt.toISOString()}', '${pickedAt.toISOString()}', ${isBreached ? 8 : 2})`);

      if (isBreached && delayMinutes > 12 && (i % 3 === 0)) {
        complaintRows.push(
          `(${i}, ${custId}, 'LATE_DELIVERY', 'Delivery delayed by ${delayMinutes} minutes beyond 30m SLA window.', '${delayMinutes > 20 ? 'CRITICAL' : 'HIGH'}', 'resolved')`
        );
        insertedComplaints++;

        if (delayMinutes > 18) {
          refundRows.push(
            `(${i}, ${custId}, ${Math.round(orderValue * 0.5 * 100) / 100}, 'SLA Breach Delay Guarantee', 'SLA_GUARANTEE')`
          );
          insertedRefunds++;
        }
      }
    ...
    // Insert complaints in 1 batch if any
    if (complaintRows.length > 0) {
      await client.query(`
        INSERT INTO complaints (delivery_id, customer_id, complaint_type, complaint_text, severity, status)
        VALUES ${complaintRows.join(',\n')};
      `);
    }

    // Insert refunds in 1 batch if any
    if (refundRows.length > 0) {
      await client.query(`
        INSERT INTO refunds (delivery_id, customer_id, refund_amount, refund_reason, refund_type)
        VALUES ${refundRows.join(',\n')};
      `);
    }
    ...
    return {
      insertedRestaurants: restaurantIds.length,
      insertedRiders: riderIds.length,
      insertedDeliveries: deliveryRows.length,
      insertedComplaints,
      insertedRefunds,
      activeAlerts: 3,
      scenario,
